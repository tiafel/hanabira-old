...
class _FilterWith(_App):
    ...
    def invoke(self, context):
        ...
        if context.next_context.object_type is APP:
            return list(filter(filtered))
        else:
            # filtering a filter
            def composed(app):
                return list(filter(filtered(app)))
            return composed
...
def loadcontext(object_type, uri, name=None, relative_to=None,
    ...
    if scheme not in _loaders:
        raise LookupError(
            ...
            % (scheme, ', '.join(list(_loaders.keys()))))
...
class ConfigLoader(_Loader):
    ...
    def get_context(self, object_type, name=None, global_conf=None):
        ...
        for local_var, glob_var in list(get_from_globals.items()):
...
class EggLoader(_Loader):
    ...
    def get_context(self, object_type, name=None, global_conf=None):
        ...
        if self.spec == "Paste":
            ...
            d = pkg_resources.get_distribution(self.spec)
    ...
    def find_egg_entry_point(self, object_type, name=None):
        ...
        if name is None:
            name = 'main'
        if name == 'http':
            return (server_runner, 'paste.server_runner', 'http')
